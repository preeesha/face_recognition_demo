...
app.post("/upload_recording", upload.single("file"), (req, res) => {
  try {
    ...
    if (!metadata.file_size_mb && req.file.size) {
      metadata.file_size_mb = parseFloat((req.file.size / (1024 * 1024)).toFixed(2));
    }
    ...
    console.log(`👥 Total Unique Persons: ${metadata.total_unique_persons || 0}`);
    console.log(`🧠 Detected Persons: ${metadata.detected_persons?.join(", ") || "None"}`);
    console.log("───────────────────────────────\n");
...
app.get("/recordings", (req, res) => {
  try {
    const files = fs.readdirSync(RECORDINGS_DIR).filter((f) => f.endsWith(".avi"));
...
app.delete("/recording/:filename", (req, res) => {
  try {
    ...
    if (fs.existsSync(metaPath)) {
      ...
      console.log(`🗑️ Deleted metadata: ${filename.replace(".avi", "_metadata.json")}`);
    }

...
import threading, time, json, requests, queue
...
recording_state = {
    ...
    "upload_status": "idle",  # values: "idle", "uploading", "success", "failed"
    "frame_queue": queue.Queue(maxsize=100),  # Shared frame queue
    "record_frames": False,  # Flag to tell camera to save frames
}
camera_state = {
    "active_streams": set(),
    "lock": threading.Lock(),
...
# ========================================================
# Recording session - Uses shared camera stream
# ========================================================
class RecordingSession:
    def __init__(self, duration=60, threshold=0.6, fps=4):
    ...
    def record_from_queue(self, rec: LiveFaceRecognizer):
        """Record video from shared frame queue"""
        ...
        try:
            while True:
                ...
                try:
                    frame_data = recording_state["frame_queue"].get(timeout=1.0)
                    frame = frame_data["frame"]
                    detected_persons = frame_data["detected_persons"]
                    ...
                    if writer is None:
                        ...
                        print(f"📹 Video writer initialized: {w}x{h} @ {self.fps} FPS")
                    
                    # Track detected persons
                    for person in detected_persons:
                        self.detected.add(person)
                        self.person_counts[person] = self.person_counts.get(person, 0) + 1
                    ...
                    print("⚠️ Frame queue empty, waiting...")
                    continue
...
def generate_frames(
    ...
    """Generate video frames for streaming AND recording"""
    ...
    try:
        ...
        while True:
            ...
            # If recording, add frame to queue
            if recording_state["record_frames"]:
                try:
                    # Create a copy for recording
                    frame_copy = frame.copy()
                    recording_state["frame_queue"].put_nowait({
                        "frame": frame_copy,
                        "detected_persons": detected_persons.copy()
                    })
                except queue.Full:
                    pass  # Skip frame if queue is full

            # Encode and yield for streaming
...
# ========================================================
def run_recording_task(duration, threshold, fps):
    """Background task for recording from shared stream"""
    try:
        with recording_state["lock"]:
            ...
            recording_state["upload_complete"] = False
            recording_state["upload_status"] = "idle"
            recording_state["record_frames"] = True
            
            # Clear frame queue
            while not recording_state["frame_queue"].empty():
                try:
                    recording_state["frame_queue"].get_nowait()
                except queue.Empty:
                    break
        ...
        rec = get_recognizer()
        sess = RecordingSession(duration, threshold, fps)
        recording_state["current_recording"] = sess

        # Start recording from queue
        sess.record_from_queue(rec)

        # Stop recording frames
        recording_state["record_frames"] = False
        ...
        if not meta["file_exists"]:
            ...
            with recording_state["lock"]:
                recording_state["upload_status"] = "failed"
            return
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                # Set success status
                with recording_state["lock"]:
                    recording_state["upload_complete"] = True
                    recording_state["upload_status"] = "success"
...
def root():
    return {
        "status": "ok",
        "message": "Pi Face Recognition Server (Shared Camera)",
...
async def start_recording(
    ...
    """Start recording from active stream"""
    
    # Check if stream is active
    with camera_state["lock"]:
        if len(camera_state["active_streams"]) == 0:
            return JSONResponse(
                {"status": "error", "message": "No active camera stream. Start video feed first."}, 
                status_code=400
            )
    ...
    background_tasks.add_task(run_recording_task, duration, threshold, fps)
...
async def recording_status():
    with recording_state["lock"]:
        ...
        upload_complete = recording_state.get("upload_complete", False)
        upload_status = recording_state.get("upload_status", "idle")
...
║  - Shared camera between stream and recording                ║
║  - Upload status tracking                                    ║
║  - Live stream visible during recording                      ║
╚═══════════════════════════════════════════════════════════════╝
    """
    )
